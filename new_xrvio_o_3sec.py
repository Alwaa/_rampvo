...
class XRVIO:
    ...
    def va_align(self):
        ...
        for k in range(N):
            ...
            # if np.linalg.norm(dp_k) < 1e-4:
            #     # skip this badly‐excited interval entirely
            #     continue
            # denom = float(dp_k.dot(dp_k))
            # if denom < 1e-8:
            #     continue
        ...
        # # After building A (shape M×(3N+4)) and b (shape M):
        # lb = np.array([-np.inf]*(3*N) + [1e-3, -np.inf, -np.inf, -np.inf])
        # ub = np.array([ np.inf]*(3*N) + [ np.inf,  np.inf,  np.inf,  np.inf])
        ...
        print("Avg. v", np.sum([np.linalg.norm(v) for v in v_list]))
    ...
    def vi_ba_g2o(self):
        ...
        max_pose_id = max(i*2 for i in idxs)
        base_lm_id  = max_pose_id + 1   # next unused ID

        self._lm_vertex_ids = {}
        # 6) Add landmark vertices
        for idx, (lm_id, P3) in enumerate(self.landmarks.items()):
            vid = base_lm_id + idx
            self._lm_vertex_ids[lm_id] = vid

            p   = g2o.VertexPointXYZ()
            p.set_id(vid)
            # p.set_estimate(np.array(P3))
            p.set_estimate(np.array(P3).reshape(3,1))
        ...
        for lm_id in self.landmarks:            
            vid = self._lm_vertex_ids[lm_id]
            xyz = opt.vertex(vid).estimate()   # this is a Vector3
            self.landmarks[lm_id] = np.array(xyz).flatten()
