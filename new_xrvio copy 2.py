...
class XRVIO:
    ...
    def vi_ba_g2o(self):
        """
        Visual–Inertial BA:
        Create a factor graph that jointly optimizes
        - pose SE3s
        - point landmarks
        - velocities
        - gyroscope bias
        using both reprojection and IMU preintegration factors.
        """
        # 1) build optimizer
        opt = g2o.SparseOptimizer()
        lin = g2o.LinearSolverDenseSE3()
        blk = g2o.BlockSolverSE3(lin)
        algo = g2o.OptimizationAlgorithmLevenberg(blk)
        opt.set_algorithm(algo)
        opt.set_verbose(False)

        # 2) Camera intrinsics parameter
        f  = float(self.K[0,0])
        pp = np.array([[self.K[0,2]],[self.K[1,2]]])
        cam = g2o.CameraParameters(f, pp, 0.0)
        cam.set_id(0)
        opt.add_parameter(cam)

        # 3) Gyro-bias vertex (optional)
        bias = g2o.VertexPointXYZ()
        bid = 100000
        bias.set_id(bid)
        bias.set_estimate(np.zeros(3))
        opt.add_vertex(bias)

        # 4) Sliding-window indices
        start = max(0, len(self.states) - self.window)
        idxs = list(range(start, len(self.states)))
        ...
        for i in idxs:
            R_i = self.states[i]['R']
            t_i = self.states[i]['t'].reshape(3,1)
            se3 = g2o.SE3Quat(R_i, t_i)
        ...
        # 6) Add landmark vertices
        for lm_id, P3 in self.landmarks.items():
            p   = g2o.VertexPointXYZ()
            vid = lm_id*100 + 50
            p.set_id(vid)
            p.set_estimate(np.array(P3))
            p.set_marginalized(True)
            opt.add_vertex(p)
        ...
        # 8) IMU preintegration (full SE3 edges only)
        for j in range(len(idxs)-1):
            i0, i1 = idxs[j], idxs[j+1]
            imu_t, acc, gyro = self.imu_slices[i1]
            pre = IMUPreintegrator(self.dt)
            ...
            eSE3 = g2o.EdgeSE3()
            eSE3.set_vertex(0, opt.vertex(i0*2))
            eSE3.set_vertex(1, opt.vertex(i1*2))
            meas = g2o.Isometry3d(pre.delta_R, pre.delta_p)
            eSE3.set_measurement(meas)
            info6 = np.eye(6)
            eSE3.set_information(info6)
            opt.add_edge(eSE3)
        ...
        # 10) Unpack back into states and landmarks
        for i in idxs:
            se3_opt = opt.vertex(i*2).estimate()
            # extract quaternion
            q = se3_opt.rotation()
        ...
        for lm_id in self.landmarks:
            vid = lm_id*100 + 50
            self.landmarks[lm_id] = opt.vertex(vid).estimate()

        print("VI-BA complete (full-SE3 IMU factors)")


    def __old_vi_ba_g2o(self):
        """
        Visual-Inertial Bundle Adjustment:
        Optimize poses, landmarks, and gyro bias using both visual reprojection and
        full IMU preintegration residuals over the sliding window.
        """
        import g2o
        # 1) build optimizer
        opt = g2o.SparseOptimizer()
        ...
        # 3) bias vertex (gyroscope bias)
        bias = g2o.VertexPointXYZ()
        bias_id = max([v.id() for v in opt.vertices().values()]) + 1
        bias.set_id(bias_id)
        bias.set_estimate(np.zeros(3))
        bias.set_fixed(False)
        opt.add_vertex(bias)
        ...
        # 5) add pose and velocity vertices
        for i in idxs:
            # SE3 pose
            R_i = self.states[i]['R']; t_i = self.states[i]['t'].reshape(3,1)
            se3 = SE3Quat(R_i, t_i)
            v = VertexSE3Expmap(); v.set_id(i*2)
            v.set_estimate(se3); v.set_fixed(i==start)
            opt.add_vertex(v)
            # velocity node (as PointXYZ)
            vel = VertexPointXYZ(); vel.set_id(i*2+1)
            vel.set_estimate(self.states[i]['v'])
            vel.set_fixed(i==start)
            opt.add_vertex(vel)

        # 6) add landmark vertices
        for lm_id, P3 in self.landmarks.items():
            p = VertexPointXYZ(); p.set_id(lm_id*1000+1)
            p.set_estimate(np.array(P3)); p.set_marginalized(True)
            opt.add_vertex(p)

        # 7) reprojection edges
        for i in idxs:
            for lm_id, uv in zip(self.landmarks.keys(), self.tracker.prev_pts):
                pid, vid = i*2, lm_id*1000+1
                e = EdgeProjectXYZ2UV()
                e.set_vertex(0, opt.vertex(vid)); e.set_vertex(1, opt.vertex(pid))
                e.set_measurement(uv); e.set_information(np.eye(2))
                e.set_parameter_id(0,0)
                opt.add_edge(e)

        # 8) IMU preintegration edges
        for j in range(len(idxs)-1):
            i0, i1 = idxs[j], idxs[j+1]
            imu_t, acc, gyro = self.imu_slices[i1]
            pre = IMUPreintegrator(self.dt); pre.integrate_batch(acc, gyro)
            dt = imu_t[-1] - imu_t[0]
            # rotation factor
            eR = g2o.EdgeSE3(); eR.set_vertex(0,opt.vertex(i0*2)); eR.set_vertex(1,opt.vertex(i1*2))
            measR = g2o.Isometry3d(pre.delta_R, np.zeros(3)); eR.set_measurement(measR)
            info6 = np.eye(6); info6[3:,3:]*=0.0; eR.set_information(info6)
            opt.add_edge(eR)
            # velocity factor: v_{i+1} = v_i + g*dt + R_i*(pre.delta_v) - bias
            ev = g2o.EdgePointXYZ()
            ev.set_vertex(0,opt.vertex(i0*2+1)); ev.set_vertex(1,opt.vertex(bias_id)); ev.set_vertex(2,opt.vertex(i1*2+1))
            ev.set_measurement(pre.delta_v)
            ev.set_information(np.eye(3)); opt.add_edge(ev)
            # position factor: p_{i+1} = p_i + v_i*dt + 0.5*g*dt^2 + R_i*(pre.delta_p) - bias*dt
            ep = g2o.EdgePointXYZ()
            ep.set_vertex(0,opt.vertex(i0*2)); ep.set_vertex(1,opt.vertex(i0*2+1));
            ep.set_vertex(2,opt.vertex(bias_id)); ep.set_vertex(3,opt.vertex(i1*2))
            ep.set_measurement(pre.delta_p)
            ep.set_information(np.eye(3)); opt.add_edge(ep)

        # 9) optimize
        opt.initialize_optimization(); opt.optimize(10)
        ...
        for i in idxs:
            ...
            self.states[i]['R'] = R_opt
            self.states[i]['t'] = t_opt
            self.states[i]['v'] = opt.vertex(i*2+1).estimate()

        self.gyro_bias = opt.vertex(bias_id).estimate()
        for lm_id in self.landmarks:
            self.landmarks[lm_id] = opt.vertex(lm_id*1000+1).estimate()
        print('VI-BA complete: bias', self.gyro_bias)

    def __vi_ba_g2o(self):
        """
