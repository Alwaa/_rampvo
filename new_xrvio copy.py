    ...
    def track(self, img):
        if self.prev_pts is not None:
            pts_cur, status, _ = cv2.calcOpticalFlowPyrLK(
                self.prev_img, img, self.prev_pts, None, **self.lk_params)
            mask = status.flatten()==1
            self.prev_pts = pts_cur[mask]
            self.prev_ids = self.prev_ids[mask]
        self.prev_img = img
        return self.prev_pts, self.prev_ids
